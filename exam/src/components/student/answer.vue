            ...
            <div class="item">
              ...
              <ul>
                <li v-for="(list, index4) in topic[4]" :key="index4">
                  <a href="javascript:;" @click="short(index4)"
                     :class="{'border': index == index4 && currentType == 4,'bg':  shortAnswer[index4] != null}"><span
                    :class="{'mark': topic[4][index4].isMark == true}"></span>{{
      ...
      <transition name="slider-fade">
        <div class="right">
          <div class="title">
            ...
            <span>全卷共{{ topicCount[0] + topicCount[1] + topicCount[2] + topicCount[3] }}题  <i
              class="iconfont icon-time"></i>倒计时：<b>{{ time }}</b>分钟</span>
          </div>
          <div class="content">
            <p class="topic"><span class="number">{{ number }}</span>{{ showQuestion }}<span style="margin-left: 20px">{{
            ...
            <div class="short" v-if="currentType == 4">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li>
                    <el-tag type="success">正确答案：</el-tag>
                    <span class="right">{{ topic[4][index].answer }}</span></li>
                  <li>
                    <el-tag>题目解析：</el-tag>
                  </li>
  ...
  methods: {
    ...
    getExamData() { //获取当前试卷所有信息
      ...
      this.$axios(`/api/exam/${examCode}`).then(res => {  //通过examCode请求试卷详细信息
        ...
        this.$axios(`/api/paper/${paperId}`).then(res => {  //通过paperId获取试题题目信息(及答案，后续提交进行判断出分数)
          this.topic = {...res.data}
