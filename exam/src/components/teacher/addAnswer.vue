    ...
    <el-table :data="pagination.records" border>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          <el-button @click="add(scope.row.paperId,scope.row.source,scope.row.totalScore,scope.row.subject)" type="primary" size="small">
            增加题库
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    add(paperId, source, totalScore,subject) { //增加题库（重定向GET传参）
      this.$router.push({
