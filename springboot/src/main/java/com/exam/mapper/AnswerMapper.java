...
//            "union select  question, subject, score, section,level, \"填空题\" as type from fill_question")
    ...
    class AnswerDao {
        public String findAll(String subjects, String type) {
            String sql = "";
            if (type.equals("选择题")) {
                sql += "select questionId, question, subject, score, section,level, '选择题' as type from multi_question WHERE subject = #{subjects}";
            } else if (type.equals("填空题")) {
                sql += "select questionId, question, subject, score, section,level, '填空题' as type from fill_question WHERE subject= #{subjects}";
            } else if (type.equals("判断题")) {
                sql += "select questionId, question, subject, score, section,level, '判断题' as type from judge_question WHERE subject= #{subjects}";
            } else if (type.equals("简答题")) {
                sql += "select questionId, question, subject, score, section,level, '简答题' as type from short_question WHERE subject =#{subjects}";
            } else {
                sql += "select questionId, question, subject, score, section,level, '选择题' as type from multi_question WHERE subject =#{subjects} " +
                        "union select questionId, question, subject, score, section,level, '判断题' as type from judge_question WHERE subject = #{subjects} " +
                        "union select questionId, question, subject, score, section,level, '填空题' as type from fill_question WHERE subject = #{subjects} " +
                        "union select questionId, question, subject, score, section,level, '简答题' as type from short_question WHERE subject =#{subjects}";
            }
            return sql;
